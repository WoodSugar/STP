...
def generate_data_all(args: argparse.Namespace):
    ...
    data_info = json.load(open(os.path.join(data_info_folder, "METR_LA"), "r"))
    ...
    speed_file = os.path.join(data_info.folder, data_info.speed_file)

    df = pd.read_hdf(speed_file)
    speed_data = np.expand_dims(df.values, axis=-1)

    merge_index = []
    for i in range(0, speed_data.shape[0], merge_number):  # [0, 2, 4, ... , 98]  
        merge_index.append([i + tau for tau in range(merge_number)])
    print("T length / merger number = ", speed_data.shape[0] / merge_number)

    speed_data = sum_byindex(speed_data, merge_index, axis=0)

    print("Raw data shape: {0}".format(speed_data.shape))
    total_days = data_info.divide_days
    speed_data = choose_data_one_day(speed_data, time_interval, data_info.one_day_range, sum(total_days))
    print("Choosed data shape: {0}".format(speed_data.shape))

    num_samples = speed_data.shape[0] - (src_len + trg_len) + 1
    ...
    data_norm = scaler(speed_data, output_dir, train_index, src_len, trg_len,
    ...
    with open(ori_graph_flie, "rb") as f:
        graph = pickle.load(f, encoding='iso-8859-1')

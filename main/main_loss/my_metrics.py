...
from basic_ts.losses.losses import masked_mse, masked_mae
...
def masked_huber_loss(preds: torch.Tensor, labels: torch.Tensor, null_val: float = 0.0) -> torch.Tensor:
    mae = masked_mae(preds, labels, null_val)
    huber = torch.where(mae < 1.0, 0.5 * mae ** 2, mae - 0.5)
    return huber
...
from compare_model.BTS_STFGNN import STFGNNSeq2Seq, construct_adj_fusion
...
CFG.DTW.ONE_DAY_LENGTH = int((CFG.DATASET_DESCRIBE.get("one_day_range")[1] - CFG.DATASET_DESCRIBE.get("one_day_range")[0]) * 60 / CFG.DATASET_DESCRIBE.get("time_interval"))
...
# ================= model ================= #
CFG.MODEL = EasyDict()
CFG.MODEL.NAME = "STFGNN_O_FT_Hyper"
...
if os.path.exists(os.path.join(CFG.DATASET_DESCRIBE.get("folder"), "graph_dtw.csv")):
    adj_dtw = pd.read_csv(os.path.join(CFG.DATASET_DESCRIBE.get("folder"), "graph_dtw.csv"), header=None).values

    local_adj = construct_adj_fusion(adj_mx, adj_dtw, steps=CFG.LOCAL_STEP)
...
CFG.MODEL.PARAM = {
    ...
    "backbone_model": STFGNNSeq2Seq,
    "backbone_path": "result/compare_model/STFGNN/PeMS04/STFGNN_Ori_PeMS04New_300/43c1fb492a9b626bc9a85f6ccfefdff5/STFGNN_Ori_best_val_MAE.pt",
    "backbone_args": {
        "config": {
...
CFG.TRAIN.CKPT_SAVE_DIR = os.path.join(
    "result/hyper_model/STFGNN/PeMS04",

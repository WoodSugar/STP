...
def train_epoch(model: nn.Module, train_loader: DataLoader, criterion: nn.MSELoss, optimizer: torch.optim.Adam,
                option, device: torch.device, ration: float, step_num: int):
    epoch_loss = 0.0

    model.train()

    re_id = np.random.permutation(range(option.num_nodes))
    for iter_num, data in enumerate(train_loader):
        optimizer.zero_grad()
        if iter_num % step_num == 0:
            re_id = np.random.permutation(range(option.num_nodes))

        prediction, target = model(data, device=device, ration=ration, re_id=re_id)

        loss = criterion(prediction[:, :, option.predict_idx], target[:, :, option.predict_idx])
        loss.backward()
...
def eval_epoch(model, valid_loader, criterion, option, device):
    epoch_loss = 0.0
...
def train(model: nn.Module, train_loader: DataLoader, valid_loader: DataLoader, criterion: nn.MSELoss(),
    ...
    for each_epoch in range(option.epoch):
        ...
        train_loss = train_epoch(model=model, train_loader=train_loader, criterion=criterion, optimizer=optimizer,
                                 option=option, device=device, ration=ration, step_num = 50)
        ...
        valid_loss = eval_epoch(model=model, valid_loader=valid_loader, criterion=criterion, option=option, device=device)

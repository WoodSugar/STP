...
from easytorch.config.utils import import_config, config_md5
...
def get_best_model_file_path(cfg: str):
    cfg = import_config(cfg)
    md5_code = config_md5(cfg)
    ckpt_path = os.path.join(cfg.TRAIN.CKPT_SAVE_DIR, md5_code)
    files = os.listdir(ckpt_path)
    model_file = list(filter(lambda file: file.endswith(".pt") and "best_val" in file, files))

    if len(model_file) == 0:
        print("No best val model found!")
        return None
    elif len(model_file) > 1:
        print("More than one best val model found!")
        return None
    else:
        ckpt_file = os.path.join(ckpt_path, model_file[0])
        return ckpt_file

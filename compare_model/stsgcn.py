...
if __name__ == '__main__':
    A = np.random.randn(278, 278)
    local_adj = construct_adj(A, 3)
    local_adj = torch.FloatTensor(local_adj)

    model = STSGCN(adj=local_adj,
           history=12,
           num_of_vertices=278,
           in_dim=2,
           out_dim=2,
           ...
           out_layer_dim=128,
           activation='GLU', use_mask=True, temporal_emb=True, spatial_emb=True, horizon=12, strides=3)

    data = torch.randn(32, 278, 12, 2)

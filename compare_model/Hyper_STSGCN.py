...
class gcn_hyper(nn.Module):
    def __init__(self, adj, in_dim, out_dim, Sk, Tk, affine, activation='GLU'):
        """
        图卷积模块
        :param adj: 邻接图
        :param in_dim: 输入维度
    ...
    def forward(self, x, mask=None):
        ...
        x = x.squeeze(1).transpose(0, 1) # [3N, B, Cin]
...
class STSGCM(nn.Module):
    def __init__(self, adj, in_dim, out_dims, num_of_vertices, Sk, Tk, affine, activation='GLU'):
        """
        :param adj: 邻接矩阵
        :param in_dim: 输入维度
        ...
        self.gcn_hypers = nn.ModuleList()

        self.gcn_hypers.append(
            gcn_hyper( #adj, in_dim, out_dim, affine, activation='GLU'
                adj=self.adj,
                in_dim=self.in_dim,
...
class output_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        out2 = self.FC2(out1)  # (B, N, hidden) -> (B, N, horizon * self.out_dim)

        del out1, batch_size
        B, N, H = out2.shape

        out2 = out2.view(B, N, self.horizon, self.out_dim)  # (B, N, horizon, self.out_dim

        return out2.permute(0, 2, 1, 3)  # B, horizon, N, out_dim


class STSGCN(nn.Module):
    def __init__(self, adj, history, num_of_vertices, in_dim, out_dim, hidden_dims,
                 first_layer_embedding_size, out_layer_dim, Sk, Tk, activation='GLU', use_mask=True,
                 temporal_emb=True, spatial_emb=True, horizon=12, strides=3, affine=True):
        """
...
class HyperSTSGCN(STSGCN):
    def __init__(self, adj, history, num_of_vertices, in_dim, out_dim, hidden_dims, first_layer_embedding_size, out_layer_dim, Sk, Tk, activation='GLU', use_mask=True, temporal_emb=True, spatial_emb=True, horizon=12, strides=3, affine=True, use_long_history=False):
        super().__init__(adj, history, num_of_vertices, in_dim, out_dim, hidden_dims, first_layer_embedding_size, out_layer_dim, Sk, Tk, activation, use_mask, temporal_emb, spatial_emb, horizon, strides, affine)

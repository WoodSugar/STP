...
class HyperGraphConv(nn.Module):
    def __init__(self, c_in, c_out, T_length, Sk, Tk, gso, affine, method="default"):
        super(HyperGraphConv, self).__init__()
        
        if method == "default":
            self.gcn = HyperNet(gso.size(0), T_length, c_in, Sk, Tk, affine=affine)
        elif method == "nFC":
            self.gcn = HyperNetnFC(gso.size(0), T_length, c_in, Sk, Tk, affine=affine)
        elif method == "nS":
            self.gcn = HyperNetnS(gso.size(0), T_length, c_in, Sk, Tk, affine=affine)
        elif method == "nT":
            self.gcn = HyperNetnT(gso.size(0), T_length, c_in, Sk, Tk, affine=affine)
        elif method == "nWb":
            self.gcn = HyperNet(gso.size(0), T_length, c_in, Sk, Tk, affine=False)
...
class STConvBlock(nn.Module):
    ...
    def __init__(self, Kt, T, n_vertex, last_block_channel, channels, act_func, Sk, Tk, gso, affine, droprate, method="defalut"):
        super(STConvBlock, self).__init__()
        self.tmp_conv1 = TemporalConvLayer(
            Kt, last_block_channel, channels[0], n_vertex, act_func)
        self.graph_conv = HyperGraphConv(
            channels[0], channels[1], T - (Kt - 1), Sk, Tk, gso, affine, method) # c_in, T_length, Sk, Tk, gso, affine
...
class STGCNHyperGraphConv(nn.Module):
    ...
    def __init__(self, Kt, blocks, T, n_vertex, act_func, Sk, Tk, gso, affine, bias, droprate, method="defalut"):
        super(STGCNHyperGraphConv, self).__init__()
        modules = []
        for l in range(len(blocks) - 3):
            modules.append(STConvBlock(
                Kt, T - l * 2 * (Kt - 1), n_vertex, blocks[l][-1], blocks[l+1], act_func, Sk, Tk, gso, affine, droprate, method))
...
class HyperSTGCN(STGCNHyperGraphConv):
    def __init__(self, Kt, blocks, T, n_vertex, act_func, Sk, Tk, gso, affine, bias, droprate, **model_kwargs):
        ...
        super().__init__(Kt, blocks, T, n_vertex, act_func, Sk, Tk, gso, affine, bias, droprate, method)
